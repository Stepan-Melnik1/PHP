...
Для анализа был выбран сайт Google. В поисковой строке был введён запрос `jews`. После выполнения поиска во вкладке Network браузера был найден HTTP-запрос, связанный с получением результатов поиска.

- **Request URL:**  
`https://www.google.com/search?q=jews&...`

- **Основная часть URL:**  
`https://www.google.com/search?q=jews`

- **Request Method: `GET`**  
Метод `GET` используется потому, что пользователь запрашивает результаты поиска у сервера. Поисковый запрос передаётся в URL в виде query-параметров.

- **Status Code: `200 OK`**  
Код `200 OK` означает, что сервер Google успешно получил и обработал запрос и вернул страницу с результатами поиска.

**Query Parameters:**

*Основными параметрами запроса являются:*
```
* `q=jews` — поисковый запрос пользователя. Параметр q содержит текст, который был введён в поисковую строку.
* `biw=610` — ширина области браузера, используемая Google для адаптации страницы.
* `bih=849` — высота области браузера.
* `sxsrf=...` — служебный защитный параметр запроса.
* `ei=...` — служебный идентификатор запроса.
* `ved=...` — служебный параметр Google, связанный с контекстом перехода.
* `uact=5` — служебный параметр, связанный с действием пользователя.
* `oq=jews` — исходный поисковый запрос.
* `sclient=gws-wiz-serp` — служебный параметр, определяющий источник или режим поиска.
``` 
Наиболее важным параметром является `q=jews`, поскольку именно он содержит поисковый запрос пользователя.

**Request Headers**  
В запросе присутствуют заголовки `Accept`, `Accept-Encoding`, `Accept-Language`, `Referer`, `User-Agent` и другие. Они сообщают серверу информацию о поддерживаемых форматах содержимого, способах сжатия, предпочитаемом языке, браузере и странице, с которой был выполнен запрос. Также присутствуют HTTP/2-псевдозаголовки `:authority`, `:method`, `:path` и `:scheme`, которые определяют сервер, метод, путь запроса и используемый протокол.

**Response Headers**  
Сервер Google возвращает заголовки `Content-Type`, `Content-Encoding`, `Cache-Control`, `Content-Security-Policy`, `Set-Cookie`, `Strict-Transport-Security` и другие. Они определяют тип и кодировку содержимого, параметры кэширования, настройки безопасности и cookies. Заголовок Content-Type: text/html; charset=UTF-8 показывает, что сервер возвращает HTML-документ в кодировке UTF-8.

**Body**  
GET-запрос не содержит отдельного тела запроса, поскольку поисковый параметр передаётся непосредственно в URL. Тело ответа содержит HTML-код страницы Google с результатами поиска.

<img width="950" height="255" alt="часть3" src="https://github.com/user-attachments/assets/3d77cb83-85d4-4a33-a055-a6dab5adf34e" />

---

## Задание 4. Составление HTTP-запросов

### GET-запрос с заголовком User-Agent
...
User-Agent — это заголовок HTTP-запроса, который сообщает серверу информацию о клиенте: тип браузера, его версию, операционную систему и т.д. Сервер может использовать эту информацию, чтобы отдать подходящую версию страницы (например, мобильную), собрать статистику по используемым браузерам или заблокировать нежелательных ботов.

### POST-запрос на создание автомобиля
...
**Какие ещё методы HTTP-запросов существуют и для чего они используются?**

- **HEAD** — аналогичен GET, но сервер возвращает только заголовки без тела ответа. Используется, чтобы проверить существование ресурса, не загружая его содержимое.
- **OPTIONS** — позволяет узнать, какие методы поддерживает конкретный эндпоинт.
- **PATCH** — используется для частичного обновления ресурса.
- **TRACE** — применяется для отладки, показывает серверу, что именно было получено от клиента.
- **CONNECT** — устанавливает туннельное соединение, обычно используется для HTTPS через прокси-сервер.

### PUT-запрос на обновление автомобиля
...
**В чём разница между PATCH и PUT запросами?**

PUT заменяет ресурс целиком: если какое-то поле не передано в теле запроса, оно может быть сброшено или обнулено. PATCH обновляет только те поля, которые явно переданы в запросе, не затрагивая остальные данные ресурса.
...
В этом запросе заголовок `Content-Type` указывает на `application/json`, однако тело запроса передано в формате `form-urlencoded` (`ключ=значение&ключ=значение`), а не в формате JSON. Это несоответствие может привести к ошибке при попытке сервера разобрать тело запроса.

Возможный вариант ответа сервера (ошибка из-за несоответствия формата тела заявленному Content-Type):
...
Альтернативный вариант, если бы сервер всё же корректно обработал данные и создал ресурс:
...
**Предположите ситуации, когда сервер может вернуть следующие HTTP-коды состояния:**

| Код | Ситуация |
|-----|----------|
| 200 OK | Запрос выполнен успешно, например обновлены данные уже существующего автомобиля |
| 201 Created | Новый автомобиль успешно создан на сервере |
| 400 Bad Request | Тело запроса некорректно: не совпадает с заявленным Content-Type, отсутствует обязательное поле или неверный формат данных (например, `year` не число) |
| 401 Unauthorized | Запрос требует аутентификации, но она не была предоставлена |
| 403 Forbidden | Пользователь аутентифицирован, но не имеет прав на создание/изменение автомобилей |
| 404 Not Found | Указанный эндпоинт (например, `/cars/1`) не существует на сервере |
| 500 Internal Server Error | На стороне сервера произошла внутренняя ошибка при обработке запроса |
